...
#6. 
data.scrape <- function(the_PWSID, the_year) {
  the_url <- ifelse(
    the_year == "2022" & the_PWSID == "03-32-010",
    'https://www.ncwater.org/WUDC/app/LWSP/report.php?pwsid=03-32-010&year=2022' ,
    paste0('https://www.ncwater.org/WUDC/app/LWSP/report.php?','pswid=', the_PWSID,
           '&year=', the_year))
  ...
  Max_Day_Use_tag <- "th~ td+ td , th~ td+ td"

  the_water_system <- the_website %>% html_nodes(water_system_tag) %>% html_text()
  the_site_ID <- the_website %>% html_nodes(PWSID_tag) %>% html_text()
  the_Ownership <- the_website %>% html_nodes(Ownership_tag) %>% html_text()
  the_max_use <- the_website %>% html_nodes(Max_Day_Use_tag) %>%  html_text()

  df_withdrawls <- data.frame("Month" = c("Jan", "May", "Sep", "Feb", "Jun", "Oct",
                                          "Mar", "Jul", "Nov", "Apr", "Aug", "Dec"),
                              "Year" = rep(the_year,12),
                            "Max_Day_Use_mgd" = as.numeric(the_max_use),
...
#7 
Durham_2015 <- data.scrape("03-32-010", "2015")
