...
vector1[3] # third element of vector 1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #sequence of numbers, basic matrix, default is organized by column
matrix1
...
matrix1[4, ] #row 4, all columns
matrix1[ , 3] #column 3, all rows
matrix1[c(12, 14)] #pointing to element 12 and 14 (counting by row until it hits the 12th element/position 12)
matrix1[c(12:14)] #pointing to positions 12-14
...
dataframe1 <- data.frame(d,e,f) #column names are automatically assigned
...
```{r DataExercise-1a}
hightemp <- c(81, 82, 83, 84, 79, 71, 78, 82, 85, 84)
lowtemp <- c(55, 56, 58, 60, 60, 62, 62, 64, 64, 62)
```
...
```{r DataExercise-1c}
forecast <- data.frame(hightemp, hightemp_C, lowtemp, lowtemp_C)
names(forecast) <- c("High Temp, F", "High Temp, C", "Low Temp, F", "Low Temp, C")
```
...
sd does not accept data frames

```{r DataExercise-1d}
summary(forecast)
sd(forecast$`High Temp, F`)
```
