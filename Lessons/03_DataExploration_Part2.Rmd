...
#changing the binwidth, so each bin is only 10 units
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#20 bins, generally want between 5 and 20 bins depending on the data
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#forcing x to go from 0 to 500
...
#
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean), bins = 30)
...
```{r, fig.height = 3, fig.width = 4}
#looking at the frequency of the mean, min and max. 
ggplot(USGS.flow.data) +
...
#modifying aesthetics so that the lines are coded by mean approval-colors each record by its mean approval (two categories of data on the same plot)
ggplot(USGS.flow.data) +
...
#different slices of data done by group command and cut_width-discharge mean by gage height mean. Can look at how dischage mean is distributed at different values of gage height mean
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plot combines range and distribution. 
ggplot(USGS.flow.data) +
...
> Answer: I learned that data collection goes back for a long time. The last scatterplot showed a break in data collection around 1980. The boxplots showed how the data are distributed between approvals and how dischagrge is distributed across gage height. The scatter plot showed the relationship between the discharge and gage height means.  


How can multiple options for data exploration inform our understanding of our data?

> Answer: Different data expolration tools show different trends and relationships in the data, so it is important to use multiple forms when performing data exploration to fully understand the data. 
...
> Answer: There seems to be a relationship between discharge and gage height, which is intuitive. Rivers tend to flow faster when they are higher. 
