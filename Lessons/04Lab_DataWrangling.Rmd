...
# dplyr filtering, depth == 0 is the query (grab all records where the depth is zero and create a new dataframe)
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0)

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename) #good practice to make sure records are entered correctly
...
#the more complicated and unnecessary
summary(NTL.phys.data$year4)
class(NTL.phys.data$year4)
#NTL.phys.data$year4 <- factor(NTL.phys.data$year4)
#levels(NTL.phys.data$year4)
...
NTL.phys.data.Tuesday <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
