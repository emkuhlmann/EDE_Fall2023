...
> Add notes here: Ecosystem parameters
...
`==` compare tospecific value
`!=` check if values are different
`<`
`<=`
`>`
`>=`
`&` multiple conditions to check if both need to be true
`|` multiple conditions to check and one or the other need to be true
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #filtering rows with this condition
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #want just two lakes (can be either or)
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") #excluding other lakes
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #selecting based on two or more elements shortcut
...
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #just change second argument
...
#can choose relevant columns only, pulling out only the desired columns
```
### Mutate
...
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments) #pulling the columns in the order that you want

# find out the start and end dates of the dataset
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #know the last row from dataset dimensions
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #better way
```
...
#each condition is separated by %/%, only have to specify dataset once
```
