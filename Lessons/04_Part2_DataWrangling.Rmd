...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #specifying dataset
  mutate(month = month(sampledate)) %>% #adding a month column
  select(lakeid:daynum, month, sampledate:comments) %>% #re-ordering the columns 
  drop_na(depth) #getting rid of NAs
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #two columns, one for name and one with concentration
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration) #reverse of what was gathered
...
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE) #copied to an object all of the files that you want to read
LitterFiles

Litter <- LitterFiles %>%
  plyr::ldply(read.csv) #pipe to read all of the files into one dataframe
```
