...
ggplot(PeterPaul.chem.nutrients, aes(x=factor(month, levels = 1:12, labels = month.abb))) + geom_bar()
...
ggplot(PeterPaul.chem.nutrients %>% filter(depth == 0), aes(x=daynum, y=temperature_C, 
                                     color=factor(year4))) +
  geom_point() +
  facet_wrap(vars(lakename), nrow = 2) + #default is adding columns
  scale_y_continuous(name = "Surface Temp, ºC") +
  labs(color = "Year")
...
ggplot(EPAair, aes(x=PM2.5, y=Ozone, color=mean_Lat)) +
