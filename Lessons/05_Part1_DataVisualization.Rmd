...
PMplot <- 
  ...
  geom_point() #color is inside aesthetics here becuase the color will correspond to the different site names (aka you want it to be another layer unlike above)
print(PMplot)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #creating separate plots for each site name
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #columns representing years and rows representing the sites
...
NvsP <-
  ...
  geom_abline(aes(slope = 16, intercept = 0)) #lm = linear model (line)
print(NvsP)
...
# Using a gathered dataset, better and easier way 
Nutrientplot7 <-   
