...
```{r}
#Exercise 1:
Plot1 <- ggplot(data = EPAair, aes(x = Ozone, fill = factor(Year))) +
  geom_density(alpha = 0.5) +
  labs(x = "Ozone AQI Value", y = "Density", fill = "") +
  scale_fill_manual(values = c("darkgreen", "darkblue"))
#fill will color the area below the lines
Plot1

Plot2 <- ggplot(data = EPAair, aes(x = factor(Year), y = Ozone)) +
  geom_boxplot() +
...
Plot3 <- ggplot(data = EPAair, aes(x = factor(Year), y = Ozone)) +
  geom_violin() +
  labs(x = "", y = "Ozone AQI Value") +
  stat_summary(geom = "text", 
               fun = max, 
...
PM2.5_O3 <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(PM2.5_O3)
...
PMbyOzone <- ggplot(data = EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_point() +
  geom_smooth(method = lm, se = TRUE) + 
  geom_text(label = "PM2.5 = 0.38384Ozone + 15.63824", x = 105, y = 13)
PMbyOzone
```
...
library(corrplot); library(plyr)
EPACorr <- EPAair %>% 
  select(mean_Lat:Ozone) %>% 
  na.omit()
EPACorr1 <- cor(EPACorr)
corrplot(EPACorr1, method = "ellipse")
corrplot.mixed(EPACorr1, upper = "ellipse")
...
OzonebyALL.regression <- lm(data = EPACorr, PM2.5 ~ Ozone + Year + mean_Lng + 
                              Month + mean_Lat + Ozone)
summary(OzonebyALL.regression)
step(OzonebyALL.regression) #best model is removing year
OzonebyALL.regression2 <- lm(data = EPACorr, PM2.5 ~ Ozone + mean_Lng + 
                              Month + mean_Lat + Ozone)
summary(OzonebyALL.regression2)
