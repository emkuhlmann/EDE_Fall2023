...
For assignment, create a sequence of dates from first to last and then left join the tables to make sure that NAs are aligned. 

```{r}

#Exercise 5: Check if there is any missing data for both temperature and rainfall series.
summary(is.na(BR_complete$Rainfall))
summary(is.na(BR_complete$Temperature_C))
...
Temp_Time <- ggplot(BR_complete, aes(x = Date, y = Temperature_C)) +
  geom_smooth(method = "lm", se = F) +
  geom_line()
Temp_Time

Rain_Time <- ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_smooth(method = "lm", se = F) +
  geom_line()
Rain_Time
```
...
Temp_TS_decomp <- stl(Temp_TS, s.window = "periodic")
plot(Temp_TS_decomp)

Rain_TS_decomp <- stl(Rain_TS, s.window = "periodic")
plot(Rain_TS_decomp)
...
Temp_TS_Components <- as.data.frame(Temp_TS_decomp$time.series[ ,1:3])
Temp_TS_Components <- mutate(Temp_TS_Components, Observed = Temp_BR_processed$Temperature_C, Date = Temp_BR_processed$Date)

Rain_TS_Components <- as.data.frame(Rain_TS_decomp$time.series[ ,1:3])
Rain_TS_Components <- mutate(Rain_TS_Components, Observed = Rainfall_BR_processed$Rainfall, Date = Rainfall_BR_processed$Date)
...
Temperature_Trend <- ggplot(Temp_TS_Components) +
  geom_line(aes(y = Observed, x = Date), size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  geom_hline(yintercept = 0, lty = 2) +
  ylab("Temperature") +
...
Rainfall_Trend <- ggplot(Rain_TS_Components) +
  geom_line(aes(y = Observed, x = Date), size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  geom_hline(yintercept = 0, lty = 2) +
  ylab("Rainfall")
...
temp_data_trend <- SeasonalMannKendall(Temp_TS)
temp_data_trend
summary(temp_data_trend)

rain_data_trend <- SeasonalMannKendall(Rain_TS)
rain_data_trend
summary(rain_data_trend)
