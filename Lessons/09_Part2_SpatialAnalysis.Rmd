...
mapView(huc8_sf, col.region = "orange", layer.name = 'HUC 8') +
  mapView(huc8_sf2, zcol = "SUBBASIN", col.region = "purple", layer.name = 'Upper Neuse')

ggplot() +
  geom_sf(data=huc8_sf, fill = 'orange') +
  geom_sf(data = huc8_sf2, fill = 'purple')
...
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) #need sparse=F

the_filter <- st_intersects(x = counties_sf_utm, y = neuse_sf, sparse = FALSE)
#dot was a placeholder for what was piped in
...
triangle <- counties_sf_utm %>% 
  filter(NAME == c("Chatham", "Durham", "Orange", "Wake"))
